...
def shopsupprosale():
    ...
    WHERE
     DATE_SUB(CURDATE(), INTERVAL 30 DAY) <= a.`payment_time`
    AND a.`basic_state`!=1
    AND e.`cat_name`="盒饭"
...
def randombooking(bookcount):
    ...
    elif 10<bookcount<30 :
        endnum=math.ceil(bookcount/4)
    elif bookcount>30:
        endnum=math.ceil(bookcount/6)
    else:
        endnum=bookcount
    while bookcount>=2:
...
def menulenbooking(bookcount,menulen):
    ...
    while bookcount>=2:
      ...
      if len(countli)+1==menulen:
          countli.append(allcount-amount)
          break
      if bookcount==1:
          countli.append(bookcount)
          break
        ...
        for shopkey in shopsupdict:
            ...
            for superi in supermenuli:
                ...
                if superi in superlist:
                    ...
                    menuprodli=supermenudict[superi]
                    prodmenucount,prodbookcountli=randombooking(newsbookc)#菜品种类数量，每样菜品分配数量
                    if prodmenucount>len(menuprodli):
                       if newsbookc<2:
                           newsbookc=newsbookc+2
