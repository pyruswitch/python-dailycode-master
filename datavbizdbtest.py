...
class datavdb():
    ...
    def dbsql (self,selectsql,account):#数据库查询
    ...
    def substr(self,mainstr, startstr, endstr):
        try:
    ...
    LIMIT 100
        ...
        allorderdata=self.dbsql(select_sql,account)#原始支付订单数据
        ...
        if allorderdata==[]:
            ...
            for ordervalue in allorderdata:
                ...
                dicti=dicti+1
        #return orderlist
        name="parkorder"+str(ns)
        #value=datavbizdb.montharea(ns)
        value=orderlist
        redisdemo.setredis(name,value,"")
        data=r.get(name)
        #time.sleep(82800)
        return data

    def ordertypecount(self,ns):#按数量统计订单类型
        select_sql='''
            SELECT
    ...
    GROUP BY a.`order_type`
        '''.format(ns)
        account={"user":"ning.wei15","pwd":"wn3333","host":"bizdb.zuolin.com","db":"ehbiz","port":"18306",}
        ordertypecount=self.dbsql(select_sql,account)#订单类型数量
    ...
    def ordertypeamount(self,ns):#按数量统计订单类型
        select_sql='''
       SELECT
    ...
    GROUP BY a.`order_type`
        '''.format(ns)
        account={"user":"ning.wei15","pwd":"wn3333","host":"bizdb.zuolin.com","db":"ehbiz","port":"18306",}
        ordertypeamount=self.dbsql(select_sql,account)#订单类型总额
    ...
    def orderchannelcount(self,ns):#按数量统计支付渠道
        select_sql='''
       SELECT
    ...
    GROUP BY a.`online_pay_style_no`
        '''.format(ns)
        account={"user":"ning.wei15","pwd":"wn3333","host":"bizdb.zuolin.com","db":"ehbiz","port":"18306",}
        orderchannnelcount=self.dbsql(select_sql,account)#订单渠道数量
        channeldict={}
        ...
        return channeldict
    def orderchannelamount(self,ns):#按金额统计支付渠道
        select_sql='''
      SELECT
    ...
    GROUP BY a.`online_pay_style_no`
        '''.format(ns)
        account={"user":"ning.wei15","pwd":"wn3333","host":"bizdb.zuolin.com","db":"ehbiz","port":"18306",}
        orderchannelamount=self.dbsql(select_sql,account)#订单渠道数量
        orderchannelamount=list(orderchannelamount)
        return orderchannelamount

    def monthamount(self,ns):#按月份统计金额
        select_sql='''
     SELECT
    ...
    GROUP BY DATE_FORMAT(a.`pay_date`,'%m')
        '''.format(ns)
        account={"user":"ning.wei15","pwd":"wn3333","host":"bizdb.zuolin.com","db":"ehbiz","port":"18306",}
        monthamount=self.dbsql(select_sql,account)#订单渠道数量
        monthamount=list(monthamount)
        return monthamount

    def monthcount(self,ns):#按月份统计单量
        select_sql='''
     SELECT
    ...
    GROUP BY DATE_FORMAT(a.`pay_date`,'%m')
        '''.format(ns)
        account={"user":"ning.wei15","pwd":"wn3333","host":"bizdb.zuolin.com","db":"ehbiz","port":"18306",}
        monthcount=self.dbsql(select_sql,account)#订单渠道数量
        monthcount=list(monthcount)
        return monthcount

    def orderamount(self,ns):#总金额
        select_sql='''
     SELECT
    SUM(a.`pay_amount`)
    FROM
    ...
    AND c.namespace_id={}
        '''.format(ns)
        account={"user":"ning.wei15","pwd":"wn3333","host":"bizdb.zuolin.com","db":"ehbiz","port":"18306",}
        orderamount=self.dbsql(select_sql,account)#订单渠道数量
        a=orderamount[0][0]
        return a

    def ordercount(self,ns):#总单量
        select_sql='''
    SELECT
    COUNT(a.`order_no`)
    FROM
    ...
    AND c.namespace_id={}
        '''.format(ns)
        account={"user":"ning.wei15","pwd":"wn3333","host":"bizdb.zuolin.com","db":"ehbiz","port":"18306",}
        ordercount=self.dbsql(select_sql,account)#订单渠道数量
        a=ordercount[0][0]
        return a

    def tasktypecount(self,ns):#任务类型分布
        select_sql='''
    SELECT
    ...
    GROUP BY title;
        ...
        tasktypecount=self.dbsql(select_sql,account)#任务类型数量
        ...
        for taskvalue in tasktypecountli:
            taskdict[dicti]={}
            taskdict[dicti]["name"]=self.substr(taskvalue[0],':"','"')
            taskdict[dicti]["value"]=str((taskvalue[1]))
            tasklist.append(taskdict[dicti])
            dicti=dicti+1
        return tasklist

    def boardvita(self,ns):#模块活跃度
        select_sql='''
    ...
    GROUP BY param;
        '''.format(ns)
        account={"user":"ehcore","pwd":"ehcore","host":"10.1.10.45","db":"ehcore","port":"3306",}
        tasktypecount=self.dbsql(select_sql,account)
        taskdict={}
        tasklist=[]
        dicti=1
    ...
    AND  YEAR(create_time)=YEAR(NOW());
        '''.format(ns)
        account={"user":"ehcore","pwd":"ehcore","host":"10.1.10.45","db":"ehcore","port":"3306",}
        taskamount=self.dbsql(select_sql,account)#任务总数
        select_sql='''
    SELECT COUNT(*) FROM eh_flow_cases
    WHERE namespace_id = {}
    AND  YEAR(create_time)=YEAR(NOW())AND status IN (0, 3, 4);
        '''.format(ns)
        closetaskamount=self.dbsql(select_sql,account)#任务类型数量
        alltaskdict={}
        #closetaskdict={}
    ...
    def daytaskorder(self,ns):#每日任务数
        select_sql='''
    SELECT
    ...
    GROUP BY create_date;
        '''.format(ns)
        account={"user":"ehcore","pwd":"ehcore","host":"10.1.10.45","db":"ehcore","port":"3306",}
        taskamount=self.dbsql(select_sql,account)#每日任务数
        taskdict={}
        tasklist=[]
        dicti=1
    ...
    def tasktyperespon(self,ns):#任务类型平均响应时间
        select_sql='''
    SELECT
    ...
    GROUP BY title;
        '''.format(ns)
        account={"user":"ehcore","pwd":"ehcore","host":"10.1.10.45","db":"ehcore","port":"3306",}
        taskrespone=self.dbsql(select_sql,account)#每日任务数
        taskdict={}
        tasklist=[]
        dicti=1
    ...
    def taskresponse(self,ns):#总平均响应时间
    ...
    WHERE namespace_id = {} AND   title IS NOT NULL AND STATUS IN (0, 3, 4);
        '''.format(ns)
        account={"user":"ehcore","pwd":"ehcore","host":"10.1.10.45","db":"ehcore","port":"3306",}
        responsetime=self.dbsql(select_sql,account)#任务类型数量
        taskdict={}
        taskdict["name"]="总平均响应时间"
        taskdict["value"]=str( round(responsetime[0][0], 1))
        return taskdict

    def pengdingtask(self,ns):#新代办任务
        select_sql='''
    SELECT
    ...
    LIMIT 100;
        '''.format(ns)
        account={"user":"ehcore","pwd":"ehcore","host":"10.1.10.45","db":"ehcore","port":"3306",}
        pengdingtask=self.dbsql(select_sql,account)#新代办任务
        taskdict={}
        tasklist=[]
        dicti=1
    ...
    GROUP BY title, STATUS;
        '''.format(ns)
        account={"user":"ehcore","pwd":"ehcore","host":"10.1.10.45","db":"ehcore","port":"3306",}
        taskstatus=self.dbsql(select_sql,account)#新代办任务
        taskdict={}
        tasklist=[]
        dicti=1
    ...
    where namespace_id={}
        '''.format(ns)
        account={"user":"ehcore","pwd":"ehcore","host":"10.1.10.45","db":"ehcore","port":"3306",}
        totalassets=self.dbsql(select_sql,account)#总资产
        assetsdict={}
        assetsdict["name"]="总资产"
    ...
    where namespace_id={}
        '''.format(ns)
        totaltax=self.dbsql(select_sql,account)#总税收
        taxdict={}
        taxdict["name"]="总税收"
    ...
    where namespace_id={}
        '''.format(ns)
        totalpeople=self.dbsql(select_sql,account)#总人数
        peopledict={}
        peopledict["name"]="总人数"
    ...
    SELECT  DISTINCT
     ...
     LIMIT 100
        '''.format(ns)
        account={"user":"ehcore","pwd":"ehcore","host":"10.1.10.45","db":"ehcore","port":"3306",}
        companyespon=self.dbsql(select_sql,account)#企业名册
        companytype=[
    "光机电",
    "商业-其他",
    "商业-超市",
    ...
    def  settledenter(self,ns):#入驻企业
     ...
     AND namespace_id={}
        '''.format(ns)
        account={"user":"ehcore","pwd":"ehcore","host":"10.1.10.45","db":"ehcore","port":"3306",}
        settlecount=self.dbsql(select_sql,account)#总资产
        enterprisedict={}
    ...
    SELECT
     ...
     YEAR(signed_time )=YEAR(NOW())and  namespace_id={}
        '''.format(ns)
        totalsign=self.dbsql(select_sql,account)#总税收
    ...
    SELECT
     ...
     YEAR(contract_end_date)=YEAR(NOW()) and  namespace_id={}
        '''.format(ns)
        totalexpired=self.dbsql(select_sql,account)#总人数
    ...
    def InviteBusiness(self,ns):#招商引资
        select_sql='''
    SELECT
    ...
    where namespace_id={}
        '''.format(ns)
        account={"user":"ehcore","pwd":"ehcore","host":"10.1.10.45","db":"ehcore","port":"3306",}
        inbus=self.dbsql(select_sql,account)#企业名册
        communidict={}
        bargaindict={}
    ...
    `eh_var_field_items`b
     ...
     GROUP BY b.`display_name`
        ...
        '''.format(ns)
        account={"user":"ehcore","pwd":"ehcore","host":"10.1.10.45","db":"ehcore","port":"3306",}
        companytype=self.dbsql(select_sql,account)#企业类型
        companytypedict={}
        companytypelist=[]
        dicti=1
    ...
    GROUP BY a.`name`
        '''.format(ns)
        account={"user":"ehcore","pwd":"ehcore","host":"10.1.10.45","db":"ehcore","port":"3306",}
        occupancyrate=self.dbsql(select_sql,account)#入住率
        occupancyratedict={}
        occupancyratelist=[]
        dicti=1
        if occupancyrate==[]:
    ...
    def rentalamount(self,ns):#年租金收入
    ...
    FROM
     ...
     AND `namespace_id`={}
        '''.format(ns)
        leasedarea=self.dbsql(select_leasedarea_sql,account)
    ...
    and `namespace_id`={}
     ...
     `namespace_id`={}
        '''.format(ns)
        account={"user":"ehcore","pwd":"ehcore","host":"10.1.10.45","db":"ehcore","port":"3306",}
        leasedarea=self.dbsql(leasedarea_sql,account)#合同签约面积
        areaamount=self.dbsql(areaamount_sql,account)#园区总面积
        monthareadict={}
        ...
        if str(leasedarea)=="[]"or str(areaamount[0][0])=="None":
            ...
            for month in data_list_todays:
                for value in leasedarea:
                    ...
                    if month in self.getBetweenDay(value[2],value[3]):
    ...
    ORDER BY taryear DESC
        '''.format(ns)
        account={"user":"ehcore","pwd":"ehcore","host":"10.1.10.45","db":"ehcore","port":"3306",}
        totalenergy=self.dbsql(select_sql,account)#能耗
        energyvaluedict={}
        energyvaluelist=[]
    ...
    def buildmonthwatermeter(self,ns) :
    ...
    def watermetercureading(self,ns):#水表读数
        select_sql='''
    ...
    GROUP BY a.`meter_id`
        '''.format(ns)
        account={"user":"ehcore","pwd":"ehcore","host":"10.1.10.45","db":"ehcore","port":"3306",}
        watermeter=self.dbsql(select_sql,account)
        watermeterdict={}
        watermeterlist=[]
        dicti=1
    ...
    def electrmetercureading(self,ns):#水表读数
        select_sql='''
    SELECT a.`meter_id`,a.`meter_name`,a.`meter_number`,(SELECT b.`current_reading` FROM`eh_energy_date_statistics`b WHERE  b.`meter_id`=a.`meter_id`AND b.`create_time`=MAX(a.`create_time`))AS "current_reading"
     FROM `eh_energy_date_statistics` a
    WHERE
    ...
    GROUP BY a.`meter_id`
        '''.format(ns)
        account={"user":"ehcore","pwd":"ehcore","host":"10.1.10.45","db":"ehcore","port":"3306",}
        electrmeter=self.dbsql(select_sql,account)
        electrmeterdict={}
        electrmeterlist=[]
        dicti=1
        #statusdict={"0":"无状态","1":"待处理","2":"已完成","3":"已延期"}
    ...
    def qualityinspetask(self,ns):#品质核查
        select_sql='''
    SELECT
    ...
    LIMIT 100
        '''.format(ns)
        account={"user":"ehcore","pwd":"ehcore","host":"10.1.10.45","db":"ehcore","port":"3306",}
        qualitytask=self.dbsql(select_sql,account)
        qualitytaskdict={}
        qualitytasklist=[]
        dicti=1
        statusdict={"0":"无状态","1":"待处理","2":"已完成","3":"已延期"}
    ...
    def equipmentinspetask(self,ns):#品质核查
        select_sql='''
    SELECT
    ...
    LIMIT 100
        '''.format(ns)
        account={"user":"ehcore","pwd":"ehcore","host":"10.1.10.45","db":"ehcore","port":"3306",}
        equipmenttask=self.dbsql(select_sql,account)
        equipmenttaskdict={}
        equipmenttasklist=[]
        dicti=1
    ...
    def todayquality(self,ns):#今日品质任务
    ...
    AND `eh_quality_inspection_tasks`.`namespace_id`={}
        '''.format(ns)
        account={"user":"ehcore","pwd":"ehcore","host":"10.1.10.45","db":"ehcore","port":"3306",}
        todayquality=self.dbsql(select_sql,account)#任务类型数量
        todayqualitydict={}
        todayqualitylist=[]
        ...
        try:
         todayqualitydict["responrate"]=round((self.todayequipment[0][2]+self.todayequipment[0][1])/(self.todayequipment[0][0]),3)
        except Exception:
            todayqualitydict["responrate"]="--"

        return todayqualitylist

    def todayequipment(self,ns):#今日巡检任务
    ...
    AND `eh_equipment_inspection_tasks`.`namespace_id`={}
        ...
        '''.format(ns)
        account={"user":"ehcore","pwd":"ehcore","host":"10.1.10.45","db":"ehcore","port":"3306",}
        todayequipment=self.dbsql(equipment_sql,account)#任务类型数量
        equipmentdict={}
        todayqualitylist=[]
    ...
    def inspectiontasktype(self,ns):#任务类型分布
    ...
    AND `eh_equipment_inspection_tasks`.`namespace_id`={}
        '''.format(ns)
        account={"user":"ehcore","pwd":"ehcore","host":"10.1.10.45","db":"ehcore","port":"3306",}
        equipmenttotal=self.dbsql(equipment_sql,account)
        tasktypedict={}
        tasktypelist=[]
        try:
    ...
    AND `eh_quality_inspection_tasks`.`namespace_id`={}
        '''.format(ns)
        qualitytotal=self.dbsql(quality_sql,account)
        try:
    ...
    def activityenrollment(self,ns):#活动报名人数
    ...
    AND `eh_activities`.`namespace_id`={}
        '''.format(ns)
        account={"user":"ehcore","pwd":"ehcore","host":"10.1.10.45","db":"ehcore","port":"3306",}
        activityenroll=self.dbsql(select_sql,account)#
        activityenrolldict={}
    ...
    COUNT(IF(eh_forum_posts.parent_post_id <>0,eh_forum_posts.`id`,NULL)),SUM(`eh_forum_posts`.`like_count`)
     ...
     WHERE   eh_forum_posts.`status` = 2 AND `eh_forums`.`namespace_id`={} ;
        ...
        '''.format(ns)
        account={"user":"ehcore","pwd":"ehcore","host":"10.1.10.45","db":"ehcore","port":"3306",}
        forumthreads=self.dbsql(select_sql,account)#
        forumthreadsdict={}
    ...
    def activityattention(self,ns):#帖子标签分布数
        select_sql='''
    SELECT ac.tag, SUM(view_count) FROM eh_forum_posts po, eh_activities ac WHERE po.id = ac.post_id AND ac.`status` = 2 AND ac.`namespace_id` ={} GROUP BY tag ORDER BY COUNT(*) DESC;
        '''.format(ns)
        account={"user":"ehcore","pwd":"ehcore","host":"10.1.10.45","db":"ehcore","port":"3306",}
        activityattention=self.dbsql(select_sql,account)
    ...
    def trendingtopics(self,ns):#话题前五
        select_sql='''
    ...
    AND `eh_forum_posts`.`status` = 2 AND `eh_forums`.`namespace_id`={}
     ORDER BY `eh_forum_posts`.view_count DESC LIMIT 5;
        '''.format(ns)
        account={"user":"ehcore","pwd":"ehcore","host":"10.1.10.45","db":"ehcore","port":"3306",}
        trendingtopics=self.dbsql(select_sql,account)
        trendingtopicsdict={}
        trendingtopicslist=[]
        dicti=1
        if trendingtopics==[]:
            ...
            for value in trendingtopics:
                ...
                if '/' in value[0]:
                  trendingtopicsdict[dicti]["type"]=self.substr((value[0]),'/','"')
                else:
    ...
    def newactivity(self,ns):#话题前五
     ...
     LIMIT 10;
        '''.format(ns)
        account={"user":"ehcore","pwd":"ehcore","host":"10.1.10.45","db":"ehcore","port":"3306",}
        newactivity=self.dbsql(select_sql,account)
        newactivitydict={}
        newactivitylist=[]
        dicti=1
        if newactivity==[]:
    ...
    def newforum(self,ns):#话题前五
     ...
     LIMIT 10;
        '''.format(ns)
        account={"user":"ehcore","pwd":"ehcore","host":"10.1.10.45","db":"ehcore","port":"3306",}
        newforum=self.dbsql(select_sql,account)
        newforumdict={}
        newforumlist=[]
        dicti=1
        if newforum==[]:
    ...
    def communityuseract(self,ns):#用户统计
        select_sql='''
    SELECT
    ...
    WHERE
     `namespace_id`=1000000
        '''
        account={"user":"ehcore","pwd":"ehcore","host":"10.1.10.45","db":"ehcore","port":"3306",}
        useract=self.dbsql(select_sql,account)
        newuser=self.dbsql(select__newuser_sql,account)
        useractdict={}
        ...
        return useractdict
    def daynewusertrend(self,ns):#每日新增
        select_sql='''
     SELECT
    ...
    ORDER BY `create_time`
        ...
        '''.format(ns)
        account={"user":"ehcore","pwd":"ehcore","host":"10.1.10.45","db":"ehcore","port":"3306",}
        usertrend=self.dbsql(select_sql,account)
        usertrenddict={}
        usertrendlist=[]
        dicti=1
        if usertrend==[]:
    ...
    def dayactusertrend(self,ns):#每日活跃
        select_sql='''
     SELECT
    ...
    def daytotalusertrend(self,ns):#每日累计
        select_sql='''
     SELECT
...
if __name__ == '__main__':
    ...
    #print(list1,'\n',list2,'\n',list3,'\n',list4,'\n',list5,'\n',list6)
    datavdb=datavdb()
    list1=datavdb.parkorder(1000000)
    print(list1)