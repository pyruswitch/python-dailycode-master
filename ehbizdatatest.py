...
def incomedata(starttime,endtime):#流水类
    ...
    AND c.`shop_no`IN("14622779991551986272","14477417463124576784","14667588471072254231")
...
def powerincomedata(starttime,endtime):#流水类
    ...
    a.`pay_date`>="{}"
    and a.`pay_date`<="{}"
    AND c.`shop_no`IN("14803348590903554653","14836694012369535164")
    AND c.`shop_name` IS NOT NULL
    AND a.`pay_status`="success"
    GROUP BY DATE_FORMAT(a.`pay_date`,'%Y-%m-%d'),c.`shop_name`,a.`pay_type`
        '''.format(starttime,endtime)
...
AND a.`basic_state`!=1
AND e.`cat_name`="盒饭"
AND a.`shop_no`IN("14622779991551986272","14477417463124576784","14667588471072254231")
